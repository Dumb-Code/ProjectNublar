...
public class LineUtils {
    ...
    public static List<Vector2d> bresenhamYCoords(double x0, double x1, double y0, double y1) {
        double dx = x1 - x0;
        double dy = y1 - y0;

        if(Math.abs(dx) < 1) { //Don't divide by less than 1
            List<Vector2d> out = Lists.newArrayList();
            for (int y = MathHelper.floor(y0); y != MathHelper.ceil(y1); y += Math.signum(dy)) {
                out.add(new Vector2d(x0, y));
            }
            return out;
        }
