...
public class DinosaurMovementSystem implements EntitySystem {
    ...
    private void ensureAnimationPlaying(Entity entity, AnimationComponent component) {
        ...
        Animation animation = entity.isInWater() ? AnimationHandler.SWIMMING : AnimationHandler.WALKING;
        if(component.getAnimation(MOVEMENT_CHANNEL) != animation) {
        ...
        if(world != null && !Minecraft.getInstance().isPaused()) {
            for (Entity entity : world.entitiesForRendering()) {
                if(entity instanceof ComponentAccess) {
                    ((ComponentAccess) entity).get(EntityComponentTypes.ANIMATION).ifPresent(a ->
                        this.ensureAnimationPlaying(entity, a)
                    );
                }
            }
        }
